...
func (s *Server) requireAdmin(scope string, handler http.HandlerFunc) http.HandlerFunc {
	return s.requireAuth(func(w http.ResponseWriter, r *http.Request) {
		...
		if !user.IsAdmin {
			writeError(w, http.StatusForbidden, "insufficient_admin_scope", "admin access required")
			return
		}
		if !HasScope(strings.Join(user.Scopes, ","), scope) {
			writeError(w, http.StatusForbidden, "insufficient_admin_scope", fmt.Sprintf("missing required scope: %s", scope))
			return
		}
		handler(w, r)
	})
